...
function run_training(model, rng, opt, epochs)
    ...
    function main(tstate::Lux.Training.TrainState, vjp, data_loader, epochs)
        ...
        run_id = string(rand(UInt))
        loss_file = open("losses_$run_id.csv", "w")
        ...
        for epoch in 1:epochs
            for (i, data) in enumerate(data_loader)
                ...
                if i % 50 == 0
                    ps_file = "tstate_ps_$run_id_epoch_$(epoch)_iter_$(i).jls"
                    st_file = "tstate_st_$run_id_epoch_$(epoch)_iter_$(i).jls"
